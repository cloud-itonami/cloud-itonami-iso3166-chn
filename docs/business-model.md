# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — China

## Classification

- Repository: `cloud-itonami-iso3166-chn`
...
- Social impact: [:chinese-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Chinese
  public contract
...
- registration walkthrough for the China Government Procurement Network
  (中国政府采购网, ccgp.gov.cn), the national tender-notice publication
  platform operated under the Ministry of Finance
- business/tax registration checklist: the Unified Social Credit Code
  (USCC), an 18-digit identifier issued by the State Administration for
  Market Regulation (SAMR) that doubles as the taxpayer identifier with
  the State Taxation Administration, verifiable through the National
  Enterprise Credit Information Publicity System (GSXT, gsxt.gov.cn)
- local-content / preferential-procurement navigation: domestic-product
  and domestic-manufacturer preference provisions under China's
  Government Procurement Law on qualifying tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Chinese-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in China).
