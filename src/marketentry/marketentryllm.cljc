(ns marketentry.marketentryllm
  ...
  the Chinese public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-domestic-entity?=" (:has-domestic-entity? e)
                        " uscc-verified?=" (:uscc-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-domestic-entity? e))
                              (:has-domestic-entity? e))
                          (or (not (:requires-uscc? e))
                              (:uscc-verified? e)))
                   0.9 0.3)}))
