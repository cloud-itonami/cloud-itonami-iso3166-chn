(ns marketentry.store
  "SSoT for the China market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Manufacturing (Shanghai)" :portal "CCGP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-domestic-entity? true :has-domestic-entity? true
             :requires-uscc? true :uscc-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CHN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "CCGP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-domestic-entity? true :has-domestic-entity? true
             :requires-uscc? true :uscc-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems (Suzhou)" :portal "CCGP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "CHN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Trading" :portal "CCGP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-domestic-entity? true :has-domestic-entity? false
             :requires-uscc? true :uscc-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CHN" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics CN" :portal "CCGP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-domestic-entity? true :has-domestic-entity? true
             :requires-uscc? true :uscc-verified? false
             :drafted? false :submitted? false
             :jurisdiction "CHN" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech CN" :portal "CCGP"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-domestic-entity? has-domestic-entity?
                               requires-uscc? uscc-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-domestic-entity?)  (assoc :engagement/requires-domestic-entity? requires-domestic-entity?)
    (some? has-domestic-entity?)       (assoc :engagement/has-domestic-entity? has-domestic-entity?)
    (some? requires-uscc?)    (assoc :engagement/requires-uscc? requires-uscc?)
    (some? uscc-verified?)    (assoc :engagement/uscc-verified? uscc-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-domestic-entity? :engagement/has-domestic-entity?
   :engagement/requires-uscc? :engagement/uscc-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-domestic-entity? (boolean (:engagement/requires-domestic-entity? m))
     :has-domestic-entity? (boolean (:engagement/has-domestic-entity? m))
     :requires-uscc? (boolean (:engagement/requires-uscc? m))
     :uscc-verified? (boolean (:engagement/uscc-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-chn"})
