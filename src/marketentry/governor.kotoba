(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a China domestic legal entity
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. China domestic legal entity-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-domestic-entity?
                                       true` (near-universal for JPN
                                       public tenders per this
                                       blueprint's own text),
                                       INDEPENDENTLY verify
                                       `:has-domestic-entity?` is
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(统一社会信用代码/統一資格/CCGP登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- domestic-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-domestic-entity? true`, INDEPENDENTLY verify
  `:has-domestic-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-domestic-entity?` ground truth (most JPN public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-domestic-entity? e))
                 (not (true? (:has-domestic-entity? e))))
        [{:rule :domestic-entity-missing
          :detail (str subject " は中国法人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- uscc-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-uscc? true`, INDEPENDENTLY check
  `:uscc-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-uscc? e))
                 (not (true? (:uscc-verified? e))))
        [{:rule :uscc-unverified
          :detail (str subject " は统一社会信用代码確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (domestic-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (uscc-unverified-violations request st)
